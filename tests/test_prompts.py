...
def test_render_inbox_section_empty():
    """Empty deliveries returns empty string."""
    assert render_inbox_section([]) == ""


def test_render_inbox_section_single_normal():
    """Single normal delivery renders without Required actions footer."""
    deliveries = [
        {
            "delivery_id": 101,
            "note_id": 50,
            "content": "Reuse shared parser in src/foo/parser.py.",
            "must_read": False,
            "status": "queued",
    ...
    assert "[delivery:101]" in result
    assert "[note:50]" in result
    assert "[must_read]" not in result
    assert "[scope:agent]" in result
    assert "from agent=agent-9" in result
    assert "  Reuse shared parser" in result
    assert "Required actions:" not in result


def test_render_inbox_section_must_read():
    """must_read delivery includes Required actions footer."""
    deliveries = [
        {
            "delivery_id": 102,
            "note_id": 44,
            "content": "Replace legacy migration path.",
            "must_read": True,
            "status": "delivered",
            "from_agent_id": "agent-7",
            "scope": "issue",
            "recipient_issue_id": "w-a1b2",
        }
    ]
    result = render_inbox_section(deliveries)
    assert "[must_read]" in result
    assert "[scope:issue issue=w-a1b2]" in result
    assert "Required actions:" in result
    assert "hive mail ack <delivery_id>" in result


def test_render_inbox_section_mixed():
    """Mixed must_read and normal deliveries both render."""
    deliveries = [
        {
            "delivery_id": 102,
            "note_id": 44,
    ...
    assert "[must_read]" in result
    assert "Required actions:" in result
    assert "Critical update." in result
    assert "FYI note." in result


def test_render_inbox_section_has_more():
    """has_more=True adds truncation notice."""
    deliveries = [
        {
            "delivery_id": 1,
            "note_id": 1,
    ...
    assert "hive mail inbox" in result


def test_render_inbox_section_system_sender():
    """from_agent_id=None shows 'from system'."""
    deliveries = [
        {
            "delivery_id": 200,
            "note_id": 99,
...
def test_build_worker_prompt_with_inbox_section():
    """build_worker_prompt includes inbox_section in output."""
    issue = {"title": "Test Issue", "description": "Test description"}
    inbox = "### Notes Inbox Update (1 pending)\n- [delivery:1][note:1][scope:agent] from agent=a1\n  Hello."
    ...
    assert "### Notes Inbox Update (1 pending)" in prompt
