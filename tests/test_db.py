...
# --- Note delivery system tests ---


def test_migration_creates_must_read_column_and_note_deliveries_table(temp_db):
    """Test that migration creates must_read column on notes and note_deliveries table."""
    # Check must_read column exists on notes table
    cursor = temp_db.conn.execute("PRAGMA table_info(notes)")
    columns = {row[1]: row for row in cursor.fetchall()}
    assert "must_read" in columns

    # Check note_deliveries table exists
    cursor = temp_db.conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='note_deliveries'")
    assert cursor.fetchone() is not None

    # Check note_deliveries columns
    cursor = temp_db.conn.execute("PRAGMA table_info(note_deliveries)")
    nd_columns = [row[1] for row in cursor.fetchall()]
    expected = ["id", "note_id", "recipient_agent_id", "recipient_issue_id", "status", "delivered_at", "read_at", "acked_at", "created_at"]
    for col in expected:
        assert col in nd_columns

    # Check indexes exist
    cursor = temp_db.conn.execute("SELECT name FROM sqlite_master WHERE type='index'")
    index_names = {row[0] for row in cursor.fetchall()}
    assert "idx_note_deliveries_note" in index_names
    assert "idx_note_deliveries_inbox" in index_names
    assert "uidx_note_deliveries_note_agent_global" in index_names
    assert "uidx_note_deliveries_note_agent_issue" in index_names
    assert "uidx_note_deliveries_note_issue_target" in index_names
    assert "idx_note_deliveries_issue_targets" in index_names


def test_add_note_with_must_read_true(temp_db):
    """Test add_note with must_read=True stores correctly."""
    note_id = temp_db.add_note(content="Important update", must_read=True, project="test")

    cursor = temp_db.conn.execute("SELECT must_read FROM notes WHERE id = ?", (note_id,))
    row = cursor.fetchone()
    assert row["must_read"] == 1


def test_add_note_with_must_read_false_default(temp_db):
    """Test add_note defaults must_read to False."""
    note_id = temp_db.add_note(content="Regular note", project="test")

    cursor = temp_db.conn.execute("SELECT must_read FROM notes WHERE id = ?", (note_id,))
    row = cursor.fetchone()
    assert row["must_read"] == 0


def test_add_note_backward_compat(temp_db):
    """Test that existing add_note calls without must_read still work."""
    # This mirrors an existing call pattern
    note_id = temp_db.add_note(issue_id=None, agent_id=None, content="Old-style note", category="discovery", project="test")
    assert isinstance(note_id, int)
...
def test_create_note_deliveries_agent_global(temp_db):
    """Test create_note_deliveries creates agent-global rows."""
    agent1 = temp_db.create_agent("agent-1")
    agent2 = temp_db.create_agent("agent-2")
    note_id = temp_db.add_note(content="Note for agents", project="test")

    count = temp_db.create_note_deliveries(note_id, to_agents=[agent1, agent2])

    assert count == 2

    # Verify rows
    rows = temp_db.conn.execute("SELECT * FROM note_deliveries WHERE note_id = ? ORDER BY recipient_agent_id", (note_id,)).fetchall()
    assert len(rows) == 2
    # Both should have NULL recipient_issue_id (agent-global)
    for row in rows:
        assert row["recipient_issue_id"] is None
        assert row["status"] == "queued"


def test_create_note_deliveries_issue_following(temp_db):
    """Test create_note_deliveries creates issue-following target rows."""
    issue1 = temp_db.create_issue("Issue 1", project="test")
    issue2 = temp_db.create_issue("Issue 2", project="test")
    note_id = temp_db.add_note(content="Note for issues", project="test")

    count = temp_db.create_note_deliveries(note_id, to_issues=[issue1, issue2])

    assert count == 2

    # Verify rows
    rows = temp_db.conn.execute("SELECT * FROM note_deliveries WHERE note_id = ? ORDER BY recipient_issue_id", (note_id,)).fetchall()
    assert len(rows) == 2
    # Both should have NULL recipient_agent_id (issue-following target)
    for row in rows:
        assert row["recipient_agent_id"] is None
        assert row["status"] == "queued"


def test_create_note_deliveries_mixed(temp_db):
    """Test create_note_deliveries with both agents and issues."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")
    note_id = temp_db.add_note(content="Note for both", project="test")

    count = temp_db.create_note_deliveries(note_id, to_agents=[agent1], to_issues=[issue1])
    assert count == 2


def test_create_note_deliveries_deduplication(temp_db):
    """Test that calling create_note_deliveries twice doesn't create duplicates."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")
    note_id = temp_db.add_note(content="Dedup test", project="test")

    # First call
    count1 = temp_db.create_note_deliveries(note_id, to_agents=[agent1], to_issues=[issue1])
    assert count1 == 2

    # Second call (should be ignored due to unique constraints)
    count2 = temp_db.create_note_deliveries(note_id, to_agents=[agent1], to_issues=[issue1])
    assert count2 == 0

    # Verify only 2 rows total
...
def test_materialize_issue_deliveries(temp_db):
    """Test materialize_issue_deliveries creates materialized rows from issue-following targets."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")
    note_id = temp_db.add_note(content="Issue-following note", project="test")

    # Create issue-following target
    temp_db.create_note_deliveries(note_id, to_issues=[issue1])

    # Materialize for agent1 on issue1
    count = temp_db.materialize_issue_deliveries(issue1, agent1, "test")
    assert count == 1

    # Verify materialized row exists
    rows = temp_db.conn.execute(
        "SELECT * FROM note_deliveries WHERE recipient_agent_id = ? AND recipient_issue_id = ?",
        (agent1, issue1),
    ).fetchall()
    assert len(rows) == 1
    ...
    # Original issue-following target should still exist
    targets = temp_db.conn.execute(
        "SELECT * FROM note_deliveries WHERE recipient_agent_id IS NULL AND recipient_issue_id = ?",
        (issue1,),
    ).fetchall()
    assert len(targets) == 1


def test_materialize_issue_deliveries_idempotent(temp_db):
    """Test materialize_issue_deliveries is idempotent (deduplication)."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")
    note_id = temp_db.add_note(content="Issue-following note", project="test")
    ...
    count1 = temp_db.materialize_issue_deliveries(issue1, agent1, "test")
    assert count1 == 1

    # Second materialization (should be no-op)
    count2 = temp_db.materialize_issue_deliveries(issue1, agent1, "test")
    assert count2 == 0


def test_materialize_issue_deliveries_reassignment(temp_db):
    """Test issue-following delivery materializes for new assignee on reassignment."""
    agent1 = temp_db.create_agent("agent-1")
    agent2 = temp_db.create_agent("agent-2")
    issue1 = temp_db.create_issue("Issue 1", project="test")
    note_id = temp_db.add_note(content="Issue-following note", project="test")
    ...
    # Materialize for agent1
    temp_db.materialize_issue_deliveries(issue1, agent1, "test")

    # Materialize for agent2 (new assignee)
    count = temp_db.materialize_issue_deliveries(issue1, agent2, "test")
    assert count == 1

    # Both agents should have materialized rows
    rows = temp_db.conn.execute(
        "SELECT DISTINCT recipient_agent_id FROM note_deliveries WHERE recipient_issue_id = ? AND recipient_agent_id IS NOT NULL",
        (issue1,),
    ).fetchall()
...
def test_get_injectable_deliveries_ordering(temp_db):
    """Test get_injectable_deliveries returns correct deliveries with must_read first."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    # Create a normal note and a must_read note
    normal_note = temp_db.add_note(content="Normal note", project="test")
    must_read_note = temp_db.add_note(content="Must read note", project="test", must_read=True)

    # Create deliveries
    temp_db.create_note_deliveries(normal_note, to_agents=[agent1])
    temp_db.create_note_deliveries(must_read_note, to_agents=[agent1])

    deliveries, has_more = temp_db.get_injectable_deliveries(agent1, issue1, "test")

    assert len(deliveries) == 2
    assert not has_more

    # must_read should come first
    assert deliveries[0]["must_read"] == 1
    assert deliveries[0]["content"] == "Must read note"
    assert deliveries[1]["must_read"] == 0
    assert deliveries[1]["content"] == "Normal note"


def test_get_injectable_deliveries_includes_agent_global_and_issue_scoped(temp_db):
    """Test get_injectable_deliveries includes both agent-global and issue-scoped deliveries."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    # Agent-global note
    global_note = temp_db.add_note(content="Agent-global note", project="test")
    temp_db.create_note_deliveries(global_note, to_agents=[agent1])

    # Issue-following note (needs materialization)
    issue_note = temp_db.add_note(content="Issue-scoped note", project="test")
    temp_db.create_note_deliveries(issue_note, to_issues=[issue1])
    temp_db.materialize_issue_deliveries(issue1, agent1, "test")

    deliveries, has_more = temp_db.get_injectable_deliveries(agent1, issue1, "test")

    assert len(deliveries) == 2
    scopes = {d["scope"] for d in deliveries}
    assert scopes == {"agent", "issue"}


def test_get_injectable_deliveries_caps_normal(temp_db):
    """Test get_injectable_deliveries caps normal deliveries at max_normal."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    # Create 8 normal notes and 2 must_read notes
    for i in range(8):
        note_id = temp_db.add_note(content=f"Normal note {i}", project="test")
        temp_db.create_note_deliveries(note_id, to_agents=[agent1])

    for i in range(2):
        note_id = temp_db.add_note(content=f"Must read note {i}", project="test", must_read=True)
        temp_db.create_note_deliveries(note_id, to_agents=[agent1])

    # With max_normal=3, should get 2 must_read + 3 normal = 5 total
    deliveries, has_more = temp_db.get_injectable_deliveries(agent1, issue1, "test", max_normal=3)

    must_read_count = sum(1 for d in deliveries if d["must_read"])
    normal_count = sum(1 for d in deliveries if not d["must_read"])

    assert must_read_count == 2  # All must_read always included
    assert normal_count == 3  # Capped at max_normal
    assert has_more is True  # 5 remaining normal deliveries


def test_get_injectable_deliveries_excludes_read_normal(temp_db):
    """Test get_injectable_deliveries excludes 'read' status normal notes."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    note_id = temp_db.add_note(content="Normal note", project="test")
    temp_db.create_note_deliveries(note_id, to_agents=[agent1])

    # Get delivery ID and mark it read
    deliveries, _ = temp_db.get_injectable_deliveries(agent1, issue1, "test")
    assert len(deliveries) == 1
    temp_db.mark_delivery_read(deliveries[0]["delivery_id"], agent1)

    # Should no longer appear in injectable
    deliveries, _ = temp_db.get_injectable_deliveries(agent1, issue1, "test")
    assert len(deliveries) == 0


def test_get_injectable_deliveries_includes_unacked_must_read(temp_db):
    """Test get_injectable_deliveries includes must_read notes until acked."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    note_id = temp_db.add_note(content="Must read note", project="test", must_read=True)
    temp_db.create_note_deliveries(note_id, to_agents=[agent1])

    # Mark as delivered
    deliveries, _ = temp_db.get_injectable_deliveries(agent1, issue1, "test")
    assert len(deliveries) == 1
    temp_db.mark_delivery_delivered(deliveries[0]["delivery_id"])

    # Still injectable (delivered but not acked)
    deliveries, _ = temp_db.get_injectable_deliveries(agent1, issue1, "test")
    assert len(deliveries) == 1
    ...
    # Still injectable (read but not acked)
    deliveries, _ = temp_db.get_injectable_deliveries(agent1, issue1, "test")
    assert len(deliveries) == 1

    # Ack it
    temp_db.mark_delivery_acked(deliveries[0]["delivery_id"], agent1)
...
def test_mark_delivery_delivered(temp_db):
    """Test mark_delivery_delivered transitions from queued to delivered."""
    agent1 = temp_db.create_agent("agent-1")
    note_id = temp_db.add_note(content="Test note", project="test")
    temp_db.create_note_deliveries(note_id, to_agents=[agent1])
    ...
    row = temp_db.conn.execute("SELECT * FROM note_deliveries WHERE id = ?", (delivery_id,)).fetchone()
    assert row["status"] == "delivered"
    assert row["delivered_at"] is not None

    # Second call should fail (already delivered)
    result2 = temp_db.mark_delivery_delivered(delivery_id)
    assert result2 is False


def test_mark_delivery_read(temp_db):
    """Test mark_delivery_read transitions from queued/delivered to read."""
    agent1 = temp_db.create_agent("agent-1")
    note_id = temp_db.add_note(content="Test note", project="test")
    temp_db.create_note_deliveries(note_id, to_agents=[agent1])
    ...
    # Can read from queued
    result = temp_db.mark_delivery_read(delivery_id, agent1)
    assert result is True
    ...
    # Cannot read again (already read)
    result2 = temp_db.mark_delivery_read(delivery_id, agent1)
    assert result2 is False


def test_mark_delivery_read_wrong_agent(temp_db):
    """Test mark_delivery_read fails for wrong agent."""
    agent1 = temp_db.create_agent("agent-1")
    agent2 = temp_db.create_agent("agent-2")
    note_id = temp_db.add_note(content="Test note", project="test")
    temp_db.create_note_deliveries(note_id, to_agents=[agent1])
    ...
    result = temp_db.mark_delivery_read(delivery_id, agent2)
    assert result is False


def test_mark_delivery_acked(temp_db):
    """Test mark_delivery_acked transitions for must_read notes."""
    agent1 = temp_db.create_agent("agent-1")
    note_id = temp_db.add_note(content="Must read note", project="test", must_read=True)
    temp_db.create_note_deliveries(note_id, to_agents=[agent1])
    ...
    result = temp_db.mark_delivery_acked(delivery_id, agent1)
    assert result is True

    row = temp_db.conn.execute("SELECT * FROM note_deliveries WHERE id = ?", (delivery_id,)).fetchone()
    assert row["status"] == "acked"
    assert row["acked_at"] is not None
    # read_at should be set via COALESCE
    assert row["read_at"] is not None


def test_mark_delivery_acked_fails_for_non_must_read(temp_db):
    """Test mark_delivery_acked fails for non-must_read notes."""
    agent1 = temp_db.create_agent("agent-1")
    note_id = temp_db.add_note(content="Normal note", project="test", must_read=False)
    temp_db.create_note_deliveries(note_id, to_agents=[agent1])
...
def test_mark_delivery_acked_wrong_agent(temp_db):
    """Test mark_delivery_acked fails for wrong agent."""
    agent1 = temp_db.create_agent("agent-1")
    agent2 = temp_db.create_agent("agent-2")
    note_id = temp_db.add_note(content="Must read note", project="test", must_read=True)
    temp_db.create_note_deliveries(note_id, to_agents=[agent1])
    ...
    # Wrong agent cannot ack
    result = temp_db.mark_delivery_acked(delivery_id, agent2)
    assert result is False


def test_get_required_unacked_deliveries(temp_db):
    """Test get_required_unacked_deliveries returns only unacked must_read deliveries."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    # Create must_read notes
    must_read_1 = temp_db.add_note(content="Must read 1", project="test", must_read=True)
    must_read_2 = temp_db.add_note(content="Must read 2", project="test", must_read=True)
    normal_note = temp_db.add_note(content="Normal note", project="test", must_read=False)

    temp_db.create_note_deliveries(must_read_1, to_agents=[agent1])
    temp_db.create_note_deliveries(must_read_2, to_agents=[agent1])
    temp_db.create_note_deliveries(normal_note, to_agents=[agent1])

    # All must_read should be returned (not the normal note)
    unacked = temp_db.get_required_unacked_deliveries(agent1, issue1)
    assert len(unacked) == 2
    contents = {d["content"] for d in unacked}
    assert "Must read 1" in contents
    assert "Must read 2" in contents
    assert "Normal note" not in contents

    # Ack one
    delivery_id = unacked[0]["delivery_id"]
    temp_db.mark_delivery_acked(delivery_id, agent1)

    # Should only return 1 now
    unacked = temp_db.get_required_unacked_deliveries(agent1, issue1)
    assert len(unacked) == 1


def test_get_required_unacked_deliveries_issue_scoped(temp_db):
    """Test get_required_unacked_deliveries includes issue-scoped must_read deliveries."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    # Issue-following must_read note
    must_read_note = temp_db.add_note(content="Issue must read", project="test", must_read=True)
    temp_db.create_note_deliveries(must_read_note, to_issues=[issue1])
    temp_db.materialize_issue_deliveries(issue1, agent1, "test")

    unacked = temp_db.get_required_unacked_deliveries(agent1, issue1)
    assert len(unacked) == 1
    assert unacked[0]["content"] == "Issue must read"
    assert unacked[0]["scope"] == "issue"


def test_get_inbox_deliveries(temp_db):
    """Test get_inbox_deliveries returns all deliveries for agent."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    note1 = temp_db.add_note(content="Note 1", project="test")
    note2 = temp_db.add_note(content="Note 2", project="test", must_read=True)

    temp_db.create_note_deliveries(note1, to_agents=[agent1])
    temp_db.create_note_deliveries(note2, to_agents=[agent1])

    # Get all inbox deliveries
    inbox = temp_db.get_inbox_deliveries(agent1, issue1)
    assert len(inbox) == 2

    # Both notes should be present (ordering may vary when created_at is identical)
    contents = {d["content"] for d in inbox}
    assert contents == {"Note 1", "Note 2"}


def test_get_inbox_deliveries_unread_only(temp_db):
    """Test get_inbox_deliveries with unread_only=True."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    note1 = temp_db.add_note(content="Normal note", project="test")
    note2 = temp_db.add_note(content="Must read note", project="test", must_read=True)
    note3 = temp_db.add_note(content="Another normal", project="test")

    temp_db.create_note_deliveries(note1, to_agents=[agent1])
    temp_db.create_note_deliveries(note2, to_agents=[agent1])
    temp_db.create_note_deliveries(note3, to_agents=[agent1])

    # Mark note1 as read
    d_row = temp_db.conn.execute("SELECT id FROM note_deliveries WHERE note_id = ?", (note1,)).fetchone()
    temp_db.mark_delivery_read(d_row["id"], agent1)

    # Unread only: should exclude the read normal note
    inbox = temp_db.get_inbox_deliveries(agent1, issue1, unread_only=True)
    assert len(inbox) == 2
    contents = {d["content"] for d in inbox}
    assert "Normal note" not in contents
    assert "Must read note" in contents
    assert "Another normal" in contents


def test_get_inbox_deliveries_unread_only_must_read_acked(temp_db):
    """Test get_inbox_deliveries unread_only excludes acked must_read notes."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    note1 = temp_db.add_note(content="Must read note", project="test", must_read=True)
    temp_db.create_note_deliveries(note1, to_agents=[agent1])

    # Ack the must_read note
    d_row = temp_db.conn.execute("SELECT id FROM note_deliveries WHERE note_id = ?", (note1,)).fetchone()
    temp_db.mark_delivery_acked(d_row["id"], agent1)

    # Unread only: acked must_read should be excluded
    inbox = temp_db.get_inbox_deliveries(agent1, issue1, unread_only=True)
    assert len(inbox) == 0

    # Without filter: should still be visible
    inbox = temp_db.get_inbox_deliveries(agent1, issue1, unread_only=False)
    assert len(inbox) == 1


def test_get_injectable_deliveries_scope_field(temp_db):
    """Test get_injectable_deliveries returns correct scope field."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")

    # Agent-global delivery
    note1 = temp_db.add_note(content="Agent note", project="test")
    temp_db.create_note_deliveries(note1, to_agents=[agent1])

    # Issue-scoped delivery
    note2 = temp_db.add_note(content="Issue note", project="test")
    temp_db.create_note_deliveries(note2, to_issues=[issue1])
    temp_db.materialize_issue_deliveries(issue1, agent1, "test")

    deliveries, _ = temp_db.get_injectable_deliveries(agent1, issue1, "test")

    agent_deliveries = [d for d in deliveries if d["scope"] == "agent"]
    issue_deliveries = [d for d in deliveries if d["scope"] == "issue"]

    assert len(agent_deliveries) == 1
    assert agent_deliveries[0]["content"] == "Agent note"
    assert agent_deliveries[0]["recipient_issue_id"] is None

    assert len(issue_deliveries) == 1
    assert issue_deliveries[0]["content"] == "Issue note"
    assert issue_deliveries[0]["recipient_issue_id"] == issue1


def test_get_injectable_deliveries_excludes_other_issue(temp_db):
    """Test get_injectable_deliveries excludes deliveries for other issues."""
    agent1 = temp_db.create_agent("agent-1")
    issue1 = temp_db.create_issue("Issue 1", project="test")
    issue2 = temp_db.create_issue("Issue 2", project="test")

    note1 = temp_db.add_note(content="Issue 1 note", project="test")
    note2 = temp_db.add_note(content="Issue 2 note", project="test")

    temp_db.create_note_deliveries(note1, to_issues=[issue1])
    temp_db.create_note_deliveries(note2, to_issues=[issue2])

    temp_db.materialize_issue_deliveries(issue1, agent1, "test")
    temp_db.materialize_issue_deliveries(issue2, agent1, "test")

    # Query for issue1 context only
    deliveries, _ = temp_db.get_injectable_deliveries(agent1, issue1, "test")
    assert len(deliveries) == 1
    assert deliveries[0]["content"] == "Issue 1 note"


def test_migration_idempotent_with_deliveries():
    """Test that migration is idempotent even with note_deliveries table."""
    import tempfile
    import os
    from hive.db import Database
    ...
    try:
        # Run migration again (should be no-op, no errors)
        db._migrate_if_needed()
        db._migrate_if_needed()

        # Verify table still exists and is functional
        note_id = db.add_note(content="Test", must_read=True)
        agent_id = db.create_agent("test-agent")
        count = db.create_note_deliveries(note_id, to_agents=[agent_id])
        assert count == 1
    finally:
        db.close()
        os.unlink(db_path)
