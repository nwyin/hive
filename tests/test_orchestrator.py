...
from hive.orchestrator import CompletionTransition, Orchestrator
...
# --- Delivery-based inbox injection tests ---


def test_prepare_inbox_for_worker_returns_rendered_section(temp_db, tmp_path):
    """Test _prepare_inbox_for_worker materializes and returns rendered section."""
    mock_opencode = MagicMock(spec=OpenCodeClient)
    orch = Orchestrator(
        db=temp_db,
        opencode_client=mock_opencode,
    ...
    # Create issue, agent, and a note with agent-global delivery
    issue_id = temp_db.create_issue("Test task")
    agent_id = temp_db.create_agent("test-agent")
    note_id = temp_db.add_note(content="Important update", project="test")
    ...
    # Verify note_delivered event was logged
...
def test_prepare_inbox_for_worker_returns_none_when_no_deliveries(temp_db, tmp_path):
    """Test _prepare_inbox_for_worker returns None when no deliveries exist."""
    mock_opencode = MagicMock(spec=OpenCodeClient)
    orch = Orchestrator(
        db=temp_db,
        opencode_client=mock_opencode,
    ...
    # No note_delivered event should be logged
    events = temp_db.get_events(issue_id=issue_id, event_type="note_delivered")
    assert len(events) == 0


def test_prepare_inbox_for_worker_marks_queued_as_delivered(temp_db, tmp_path):
    """Test _prepare_inbox_for_worker marks queued deliveries as delivered."""
    mock_opencode = MagicMock(spec=OpenCodeClient)
    orch = Orchestrator(
        db=temp_db,
        opencode_client=mock_opencode,
    ...
    issue_id = temp_db.create_issue("Test task")
    agent_id = temp_db.create_agent("test-agent")
    note_id = temp_db.add_note(content="Update from peer", project="test")
    temp_db.create_note_deliveries(note_id, to_agents=[agent_id])

    # First call: should mark queued -> delivered
    orch._prepare_inbox_for_worker(agent_id, issue_id)

    # Verify delivery status changed to 'delivered'
    deliveries, _ = temp_db.get_injectable_deliveries(agent_id, issue_id, "test")
    assert len(deliveries) == 1
    assert deliveries[0]["status"] == "delivered"


def test_prepare_inbox_for_worker_materializes_issue_targets(temp_db, tmp_path):
    """Test _prepare_inbox_for_worker materializes issue-following targets."""
    mock_opencode = MagicMock(spec=OpenCodeClient)
    orch = Orchestrator(
        db=temp_db,
        opencode_client=mock_opencode,
    ...
    # Create a note with issue-following target (not agent-global)
    note_id = temp_db.add_note(content="Issue-scoped note", project="test")
    temp_db.create_note_deliveries(note_id, to_issues=[issue_id])

    # _prepare_inbox_for_worker should materialize the target for this agent
    result = orch._prepare_inbox_for_worker(agent_id, issue_id)

    assert result is not None
    assert "Issue-scoped note" in result


@pytest.mark.asyncio
async def test_completion_gate_blocks_on_unacked_must_read(temp_db, tmp_path):
    """Test that completion is blocked when unacked must_read notes exist."""
    mock_opencode = AsyncMock(spec=OpenCodeClient)
    mock_opencode.get_messages = AsyncMock(return_value=[])
    orch = Orchestrator(
        db=temp_db,
    ...
    # Create a must_read note targeted at this agent
    note_id = temp_db.add_note(content="Critical update", project="test", must_read=True)
    temp_db.create_note_deliveries(note_id, to_agents=[agent_id])
    ...
    assert decision.result.success is False
    assert "required note(s) not acknowledged" in decision.result.reason
    assert "hive mail ack" in decision.result.reason

    # Verify completion_blocked_unacked_notes event was logged
...
@pytest.mark.asyncio
async def test_completion_gate_passes_when_notes_acked(temp_db, tmp_path):
    """Test that completion proceeds when all must_read notes are acknowledged."""
    mock_opencode = AsyncMock(spec=OpenCodeClient)
    mock_opencode.get_messages = AsyncMock(return_value=[])
    orch = Orchestrator(
        db=temp_db,
    ...
    # Create a must_read note and acknowledge it
    note_id = temp_db.add_note(content="Critical update", project="test", must_read=True)
    temp_db.create_note_deliveries(note_id, to_agents=[agent_id])

    # Get the delivery ID and ack it
    deliveries, _ = temp_db.get_injectable_deliveries(agent_id, issue_id, "test")
    assert len(deliveries) == 1
    temp_db.mark_delivery_acked(deliveries[0]["delivery_id"], agent_id)

    # Completion should NOT be blocked by notes (it may still fail for other reasons)
    decision = await orch._decide_completion_transition(agent)

    # Should not be blocked by unacked notes
    assert decision.transition != CompletionTransition.FAIL_ASSESSMENT or (decision.result and "required note" not in decision.result.reason)
...
@pytest.mark.asyncio
async def test_completion_gate_blocks_issue_following_must_read(temp_db, tmp_path):
    """Test completion gate blocks on issue-following must_read notes."""
    mock_opencode = AsyncMock(spec=OpenCodeClient)
    mock_opencode.get_messages = AsyncMock(return_value=[])
    orch = Orchestrator(
        db=temp_db,
    ...
    # Create a must_read note with issue-following target
    note_id = temp_db.add_note(content="Critical issue note", project="test", must_read=True)
    temp_db.create_note_deliveries(note_id, to_issues=[issue_id])

    # The completion gate should materialize the target and block
    decision = await orch._decide_completion_transition(agent)
