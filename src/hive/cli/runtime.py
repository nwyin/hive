...
def do_setup(project_path: Path, project_name: str):
    ...
    target.write_text(
        ...
        f'refinery_backend = "codex"    # codex 5.4 refinery\n'
        f'codex_reasoning_effort = "medium"       # worker thinking effort\n'
...
def do_seed_queen_files(project_path: Path):
...
def do_analyze(project_path: Path, project_name: str):
    ...
    try:
        ...
        print(toon.toon_error("Claude CLI not found. Install `claude` and ensure it's on PATH, or set CLAUDE_CMD."))
        return

    if result.returncode != 0:
        print(toon.toon_error(f"Analysis failed (exit code {result.returncode})"))
        return

    if context_path.exists():
        print(toon.encode({"context_created": str(context_path)}))
    else:
        print(toon.toon_error("Analysis completed but .hive/project-context.md was not written. Check Claude output."))
