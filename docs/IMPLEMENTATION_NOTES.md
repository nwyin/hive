...
- Machine-readable **TOON** output on all commands, auto-detected when stdout is piped/captured (force with `--toon` / `--human`)
- `hive logs -f` for live event tailing (one TOON block per event in machine mode)
